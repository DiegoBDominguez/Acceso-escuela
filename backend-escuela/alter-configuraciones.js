...
async function alterConfiguracionesTable() {
  try {
    console.log('Alterando tabla configuraciones...');

    // Agregar columnas si no existen
    await db.query(`
      ALTER TABLE configuraciones
      ADD COLUMN IF NOT EXISTS inicio_semestre DATE DEFAULT NULL,
      ADD COLUMN IF NOT EXISTS fin_semestre DATE DEFAULT NULL
    `);

    console.log('✅ Columnas agregadas a configuraciones');
    ...
    const inicioSemestre = lastWeek.toISOString().split('T')[0];
    ...
    console.log('Configuraciones:', settings[0]);
